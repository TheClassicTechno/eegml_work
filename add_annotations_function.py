...
def add_annotations(eeg_num):
    ...
    dfaeeg, dfbeeg, dfceeg = (
        dfa[str(eeg_num)].dropna(),
        dfb[str(eeg_num)].dropna(),
        dfc[str(eeg_num)].dropna(),
    )
    ...
    shutil.copy(
        eeg_file_path + f"eeg{eeg_num}.eeg.h5",
        eeg_annot_path + f"eeg{eeg_num}.annot.eeg.h5",
    )  # !cp eeg{eeg_num}.eeg.h5 eeg{eeg_num}.annot.eeg.h5
    ...
    rec.attrs["end_isodatetime"] = dateutil.parser.parse(
        rec.attrs["end_isodatetime"]
    ).isoformat()
    rec.attrs["end_isodatetime"]

    # %%
    rec.attrs["start_isodatetime"] = dateutil.parser.parse(
    ...
    # %%
    patient_clin = clinical.iloc[
        eeg_num - 1
    ]  # remember subject and eeg numbering starts at 1
    patient_clin
    ...
    # %%
    pt.attrs["gender"] = (
        patient_clin.Gender.upper() if type(patient_clin.Gender) == str else "U"
    )
    pt.attrs["gender"]
